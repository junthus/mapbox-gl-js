...
function updateVariableAnchors(
    ...
    for (const coord of coords) {
        ...
        const pixelToTileScale = pixelsToTileUnits(
            tile,
            1,
        ...
        const updateTextFitIcon =
            layer.layout.get("icon-text-fit") !== "none" &&
            bucket.hasIconData();
        ...
        if (size) {
            ...
            const getElevation = elevation
                ? (p) => elevation.getAtTileOffset(coord, p.x, p.y)
                : (_) => 0;
...
function updateVariableAnchorsForBucket(
    ...
    for (let s = 0; s < placedSymbols.length; s++) {
        ...
        const skipOrientation =
            bucket.allowVerticalPlacement && !symbol.placedOrientation;
        const variableOffset =
            !symbol.hidden && symbol.crossTileID && !skipOrientation
                ? variableOffsets[symbol.crossTileID]
                : null;

        if (!variableOffset) {
            // These symbols are from a justification that is not being used, or a label that wasn't placed
            // so we don't need to do the extra math to figure out what incremental shift to apply.
            symbolProjection.hideGlyphs(
                symbol.numGlyphs,
            ...
            const projectedAnchor = symbolProjection.project(
                tileAnchor,
                pitchWithMap ? posMatrix : labelPlaneMatrix,
                elevation
            );
            ...
            let renderTextSize =
                (symbolSize.evaluateSizeForFeature(
                    bucket.textSizeData,
                    size,
                    symbol
                ) *
            ...
            const shiftedAnchor = pitchWithMap
                ? symbolProjection.project(
                      tileAnchor.add(shift),
                      labelPlaneMatrix,
                      elevation
                  ).point
                : projectedAnchor.point.add(
                      rotateWithMap ? shift.rotate(-transform.angle) : shift
                  );

            const angle =
                bucket.allowVerticalPlacement &&
                symbol.placedOrientation === WritingMode.vertical
                    ? Math.PI / 2
                    : 0;
    ...
    if (updateTextFitIcon) {
        ...
        bucket.icon.dynamicLayoutVertexBuffer.updateData(
            dynamicIconLayoutVertexArray
        );
    }
    bucket.text.dynamicLayoutVertexBuffer.updateData(
        dynamicTextLayoutVertexArray
    );
}

function getSymbolProgramName(
    isSDF: boolean,
    isText: boolean,
    bucket: SymbolBucket
) {
...
function drawLayerSymbols(
    ...
    const pitchWithMap = pitchAlignment === "map";
    const alongLine =
        rotateWithMap && layer.layout.get("symbol-placement") !== "point";
    ...
    const defines =
        painter.terrain && pitchWithMap ? ["PITCH_WITH_MAP_TERRAIN"] : null;
    ...
    for (const coord of coords) {
        ...
        const programConfiguration = buffers.programConfigurations.get(
            layer.id
        );
        ...
        const program = painter.useProgram(
            getSymbolProgramName(isSDF, isText, bucket),
        ...
        if (isText) {
            ...
            if (bucket.iconsInText) {
                ...
                const zoomDependentSize =
                    sizeData.kind === "composite" || sizeData.kind === "camera";
                atlasInterpolationIcon =
                    transformed ||
                    painter.options.rotating ||
                    painter.options.zooming ||
                    zoomDependentSize
                        ? gl.LINEAR
                        : gl.NEAREST;
            }
            ...
            const iconScaled =
                layer.layout.get("icon-size").constantOr(0) !== 1 ||
        ...
        const labelPlaneMatrixInv =
            painter.terrain && pitchWithMap && alongLine
                ? mat4.invert(new Float32Array(16), labelPlaneMatrix)
                : identityMat4;
        ...
        const matrix = painter.translatePosMatrix(
            ...
            uLabelPlaneMatrix =
                alongLine || (isText && variablePlacement) || updateTextFitIcon
                    ? identityMat4
                    : labelPlaneMatrix,
            uglCoordMatrix = painter.translatePosMatrix(
                glCoordMatrix,
                tile,
                translate,
                translateAnchor,
        ...
        const hasHalo =
            isSDF &&
            layer.paint
                .get(isText ? "text-halo-width" : "icon-halo-width")
                .constantOr(1) !== 0;
    ...
    for (const segmentState of tileRenderState) {
        ...
        if (state.atlasTextureIcon) {
            ...
            if (state.atlasTextureIcon) {
                state.atlasTextureIcon.bind(
                    state.atlasInterpolationIcon,
                    gl.CLAMP_TO_EDGE
                );
            }
        }
